 ...
 * viewer (/api/attachment-view) — the classic exploit vectors stay maximally
 * contained.
 *
 * Same-origin isolation is layered, not absolute — stated plainly:
 ...
 *    connect-src to the /file-viewer/ ASSET PATH — a compromised renderer on
 *    the main thread cannot fetch /api/* even though it shares the origin;
 *  - the session cookie is HttpOnly — script can never read it;
 ...
 *    fetches. Accepted trade for zero extra domains; the scan gate + pinned
...
export const GET: RequestHandler = ({ url, locals }) => {
  ...
  const directives = [
    ...
    // Path-scoped: renderer asset/wasm fetches ONLY — /api/* is unreachable
