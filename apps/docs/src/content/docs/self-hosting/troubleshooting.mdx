...
`ORIGIN` doesn't match the URL the app is actually served on. better-auth
mounts its routes at `ORIGIN`. In dev, set it to `http://localhost:5173` (the
default dev port); in production, to your real app URL. Redeploy after changing.
