...
when stored**, so a database or storage leak can't read them. Search works over
scrambled one-way tokens, so [even search](/guide/search) can't see your words.
Images in a message load *through* Doota, so senders can't track you by opening
their mail.
